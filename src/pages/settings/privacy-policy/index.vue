...
const loadContent = async () => {
  ...
  try {
    ...
    const policy = sysConfStore.getConf(SysConfKeyEnum.PRIVACY_POLICY)
    
    if (policy) {
      content.value = policy
    } else {
      content.value = '<p style="color: #86868b; padding: 40rpx 0; text-align: center;">暂无隐私政策内容</p>'
    }
  } catch (error) {
